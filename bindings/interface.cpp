...
void bind_interface(py::module_ &m) {
    py::class_<usbipdcpp::UsbInterface>(m, "UsbInterface")
            ...
            .def_property_readonly("endpoints",
                                   [](const usbipdcpp::UsbInterface &self) { return self.current_endpoints(); })
            .def("set_handler",
                 [](usbipdcpp::UsbInterface &self, std::shared_ptr<usbipdcpp::VirtualInterfaceHandler> handler) {
                     self.handler = handler;
                 })
            .def(
                    "add_endpoint",
                    [](usbipdcpp::UsbInterface &self, const usbipdcpp::UsbEndpoint &ep) {
                        if (self.endpoints.empty())
                            self.endpoints.resize(1);
                        self.current_endpoints().push_back(ep);
                    },
                    py::arg("endpoint"));
}
